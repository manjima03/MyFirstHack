Day 10 of 90 with @MyFirstHack. Today was about data — the thing every breach is actually about, underneath all the technical detail.

I mapped where my personal data lives across three categories: PII, credentials, and proprietary. I found Facebook still holding my information, which I hadn't thought about in years.

I also requested a Google data export — a right I've had under GDPR for years and never used. Ten days into this course and I've already exercised a legal right most people don't know they have.

I had totally forgotten about my Google+ account and all the data I had shared on there. Will have to check if the data is stored somewhere given the site has been discontinued. 

Ten days in. The lens is starting to feel like a habit.

80 days left.

#myfirsthack
